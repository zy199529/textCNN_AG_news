...
class BiLSTM(object):
    ...
    def __init__(self, config, wordEmbedding):
        ...
        with tf.name_scope("LSTM"):
            cell = tf.contrib.rnn.BasicLSTMCell(config.model.hidden_dim)
            Cell = tf.contrib.rnn.DropoutWrapper(cell, self.dropoutKeepProb)
            output, _ = tf.nn.dynamic_rnn(cell=Cell, inputs=self.embeddedWords, dtype=tf.float32)

        with tf.name_scope("output"):
            weight = tf.Variable(tf.truncated_normal([config.model.hidden_dim, config.numClasses]))
            bias = tf.Variable(tf.constant(0.1, shape=[config.numClasses]))
            output = tf.transpose(output, [1, 0, 2])
            last = tf.gather(output, int(output.get_shape()[0]) - 1)
            self.logits = tf.nn.xw_plus_b(last, weight, bias, name="predictions")
